...
def parse_arguments():
    ...
    parser = ArgumentParser(description="Filter PAF files with confidence scores")
    parser.add_argument('-i', '--input', dest='paf_folder', required=True,
                        help='Path to folder containing PAF files')
    parser.add_argument('-o', '--output', dest='out_file', required=True,
                        help='Output file path for filtered OTU table')
    parser.add_argument('-c', '--confidence', dest='min_conf', type=int, default=1,
                        help='Minimum confidence threshold (default: 1)')
